...
const ProfileNavLinks = ({ links, activeNavStyles, notNavStyles, text }) => {
  ...
  return (
    <ul className={`h-max flex flex-col gap-[15px] md:gap-4 text-${text} divShadow text-nowrap`}>
        ...
        return (
          <li className="px-4 py-3" key={index}>
            <NavLink
              to={url}
              className={({ isActive }) =>
                `rounded flex gap-[10px] p-3 w-full ${
                  isActive ? activeNavStyles : notNavStyles
                }`
              }
            >
